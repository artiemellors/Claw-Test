...
describe("agent event handler", () => {
  ...
  describe("spawnedBy enrichment in chat and agent broadcasts", () => {
    ...
    it("resolves spawnedBy from session row only once per sessionKey", () => {
      vi.mocked(loadGatewaySessionRow).mockReturnValue({
        key: "agent:coder:subagent:cached",
        kind: "direct",
        updatedAt: null,
        spawnedBy: "agent:conductor:task:parent-cached",
      });

      const { broadcast, handler, chatRunState } = createHarness({
        resolveSessionKeyForRun: () => "agent:coder:subagent:cached",
      });

      chatRunState.registry.add("run-cache-test", {
        sessionKey: "agent:coder:subagent:cached",
        clientRunId: "client-cache-test",
      });
      ...
      for (const [, payload] of chatCalls) {
        expect(payload.spawnedBy).toBe("agent:conductor:task:parent-cached");
      }
