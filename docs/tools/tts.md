...
OpenClaw can convert outbound replies into audio using ElevenLabs, Microsoft, MiniMax, OpenAI, or Typecast.
...
If you want OpenAI, ElevenLabs, MiniMax, or Typecast:
...
- `provider`: speech provider id such as `"elevenlabs"`, `"microsoft"`, `"minimax"`, `"openai"`, or `"typecast"` (fallback is automatic).
...
- `provider` (registered speech provider id, for example `openai`, `elevenlabs`, `minimax`, `microsoft`, or `typecast`; requires `allowProvider: true`)
