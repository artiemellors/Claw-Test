...
#   ./scripts/deploy-client.sh smithlegal mel
#
# After deploying, set secrets:
#   fly secrets set ANTHROPIC_API_KEY=sk-ant-xxx --app agent-drchen
#   fly secrets set TELEGRAM_BOT_TOKEN=xxx --app agent-drchen
#
# Then SSH in to configure the agent:
...
echo ""
echo "2. Set channel token (Telegram example):"
echo "   fly secrets set TELEGRAM_BOT_TOKEN=xxx --app ${APP_NAME}"
echo ""
echo "3. SSH in and configure the agent:"
echo "   fly ssh console --app ${APP_NAME}"
echo "   openclaw config set agent.model anthropic/claude-haiku-4-5"
echo "   openclaw config set agent.systemPrompt \"Your prompt here\""
echo ""
echo "4. Verify:"
